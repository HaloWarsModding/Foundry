...
namespace Foundry.Triggerscript
{
    ...
    {
        public static IEnumerable<int> ValidVersions(EffectType type)
        {
            if (!EffectItems.ContainsKey((int)type)) return new List<int>() { -1 };
            return EffectItems[(int)type].Keys.ToList();
        }
        public static IEnumerable<int> ValidVersions(ConditionType type)
        {
            if (!ConditionItems.ContainsKey((int)type)) return new List<int>() { -1 };
            return ConditionItems[(int)type].Keys.ToList();
        }
    ...
    public class Var
    {
        ...
        private string TypeSer
        {
            ...
            set { Type = GetTypeFromString(value); }
        }
        ...
        public static VarType GetTypeFromString(string varTypeName)
        {
    ...
    public class ILogic
    {
        public ILogic()
        {
            Inputs = new List<LogicParam>();
            Outputs = new List<LogicParam>();
        }
        public ILogic(ILogic copy) : this()
        {
            this.ID = copy.ID;
            //this.Type = copy.Type;
            //this.DBID = copy.DBID;
            ...
            this.Inputs = copy.Inputs.ConvertAll(p => new LogicParam(p));
            this.Outputs = copy.Outputs.ConvertAll(p => new LogicParam(p));
        }
    ...
    [YAXSerializableType(FieldsToSerialize = YAXSerializationFields.AllFields)]
    public class Effect : ILogic
    {
        public Effect() : base()
        {

        }
        public Effect(Effect copy) : base(copy)
        {
        ...
        public EffectType Type
        {
            ...
            set
            {
                if (!Database.ValidVersions(Type).Contains(Version))
                {
                    _Version = Database.ValidVersions(Type).First();
                }
                _Type = value;
            }
        }
        [YAXDontSerialize()]
        private EffectType _Type { get; set; }
        ...
        public int Version
        {
            ...
            set
            {
                if (Database.ValidVersions(Type).Contains(value))
                    _Version = value;
                else
                    _Version = -1;
            }
    ...
    public class Condition : ILogic
    {
        ...
        public Condition(Condition copy) : base(copy)
        {
    ...
    public class Triggerscript
    {
        public Triggerscript()
        {
            ...
            TriggerVars = new Dictionary<int, Var>(); //new List<TriggerVarClass>(); //new Dictionary<int, TriggerVarClass>();
